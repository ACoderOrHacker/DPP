...
gExpressions
    : importLib #importExpr |
      operatorDefine #operatorDefineExpr | function #functionExpr  // Done
      |
      varDefine #varDefineExpr  // Done
      |
      enum #enumExpr  // Done
      |
      class #classExpr | interface_ #interfaceExpr | semi #gSemiExpr  // Done
      |
      typedef #typedefExpr;  // Done

// all expressions
expressions : new #newExpr  // Done
              |
              ...
              |
              withStatement #withExpr | whileLoop #whileLoopExpr  // Done
              |
              doWhileLoop #doWhileLoopExpr  // Done
              |
              foreachLoop #foreachLoopExpr | goto #gotoExpr  // Done
              |
              gotoLabelDefine #gotoLabelDefineExpr  // Done
              |
              try
#tryExpr | throw #throwExpr | typedef #typedefExprTag  // Done
...
constant : IntegerData #integerExpr  // Done
           |
...
data : constant #constantExpr  // Done
       |
       idEx #idExExpr  // Done
       |
       ...
       |
       LeftParen data
           RightParen #parens  // simple: (1+2)/3 Change priorities // Done
       |
       ...
       |
       data(Equal | NotEqual) data
#equalOrNotEqualExpr  // simple: 1 == 1, 2 != 1 // Done
       |
       data(Star | Div | Mod) data
#starClassExpr  // simple: 1 * 2, 1 / 2, 1 % 2 // Done
       |
       data(Plus | Minus) data #plusClassExpr  // simple: 1 + 2, 1 - 2 // Done
       |
       data(LeftShift | RightShift) data
#leftOrRightShiftExpr  // simple: 1 << 2, 1 >> 2 // Done
       |
       data(Less | Greater | LessEqual | GreaterEqual) data
#lessClassExpr  // Done
       ...
       |
       lambdaFunction #lambdaFunctionExpr | theType #typeExpr  // Done
       |
       new #newExpr_  // Done
       |
...
lambdaFunction : LeftBracket RightBracket LeftParen paramList
                 ? RightParen block;

functionHead : Fun info* ID LeftParen paramList
               ? RightParen Colon theType throwtable
               ? ;

functionCall : idEx LeftParen callParamList ? RightParen;

// TODO: If change the varDefineNoSet to varDefine, it will has bug, the auto
// value will always be the paramter but not the user set
...
whenSwitchStatement : When data LeftBrace caseStatement* defaultStatement
                      ? RightBrace;
...
foreachLoop : Foreach LeftParen(varDefine | idEx) Colon data RightParen block
              ? ;
...
operatorDefine : Operator allOperators LeftParen paramList
                 ? RightParen Arrow theType block
                 ? ;

block : LeftBrace expressions* RightBrace;

try : Try block(Catch LeftParen(varDefine | Ellipsis) RightParen block)
    *(Finally block) ? ;
...
theTypeSub : Int | Float | String | Bool | Type | Object | Void |
             ID;  // the user define types

idEx : (ID Dot)*ID;

// function or variable info
info : Compiletime | Final | Override | Inline | Static | Public | Protected |
       Private | Constructor | Destructor | native;

// simple: native("library", "function")
native : Native LeftParen StringData Comma StringData RightParen;

assigns : Assign | ColonAssign  // move the ref
          |
          DivAssign | ModAssign | PlusAssign | MinusAssign | StarAssign |
          XorAssign | AndAssign | OrAssign | LeftShiftAssign | RightShiftAssign;

allOperators : Arrow | Plus | Minus | Star | Div | Mod | Caret | And | Or |
               Tilde | Not | Assign | ColonAssign | Less | Greater |
               PlusAssign | MinusAssign | StarAssign | DivAssign | ModAssign |
               XorAssign | AndAssign | OrAssign | LeftShiftAssign |
               RightShiftAssign | Equal | NotEqual | LessEqual | GreaterEqual |
               AndAnd | OrOr | PlusPlus | MinusMinus | Colon | Dot | LeftShift |
               RightShift;